...
// lib/db.ts 의 SpeciesRow 에 아래 컬럼이 선언돼 있지 않다 (DB 에는 존재).
// 원본 타입을 고치지 않고 이 스크립트 안에서만 보강한다.
type Row = SpeciesRow & {
...
function parseCriteria(s: string | null): Record<string, Set<string>> {
...
function currentR(s: Row): { r: number; src: string } {
